...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..', 'raspberry_pi', 'client'))
...
def generate_test_scenarios():
    ...
    scenarios['clipped'] = {
        'name': 'Clipped Signal',
        'audio': np.clip(0.8 * np.sin(2 * np.pi * 440 * t) + 0.3 * np.random.randn(len(t)), -0.99, 0.99),
...
def run_enhancement_test():
    ...
    enhanced_scenarios = sum(1 for r in results.values() if r['metrics'].get('enhancement_applied', False))
...
def create_visual_comparison(results):
    ...
    try:
        ...
        for i, scenario_id in enumerate(test_scenarios):
            ...
            axes[i, 0].plot(t[:plot_samples], scenario['scenario']['audio'][:plot_samples], 'b-', alpha=0.7, label='Original')
            axes[i, 0].set_title(f"{scenario['scenario']['name']} - Original")
            axes[i, 0].set_ylabel('Amplitude')
            axes[i, 0].grid(True, alpha=0.3)
            
            axes[i, 1].plot(t[:plot_samples], scenario['scenario']['audio'][:plot_samples], 'b-', alpha=0.3, label='Original')
            # Enhanced audio would be plotted here if we stored it
