    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <button
          ...
          className="relative p-4 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-full shadow-2xl hover:shadow-3xl transform hover:scale-110 transition-all duration-300"
        >
...
function PopupNotification({ notification, onClose }: PopupNotificationProps) {
  ...
  return (
    <div className={`${bgColor} rounded-xl shadow-2xl border-2 p-4 transform transition-all duration-500 animate-in slide-in-from-right fade-in`}>
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3 flex-1">
          {/* Анимированная иконка */}
          <div className={`p-3 rounded-full ${bgColor} animate-pulse`}>
            <span className="text-3xl animate-bounce">{soundIcon}</span>
          </div>
          
          {/* Контент */}
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              {isCritical && <AlertTriangle className={`w-5 h-5 ${iconColor} animate-pulse`} />}
              {isImportant && <AlertCircle className={`w-5 h-5 ${iconColor} animate-pulse`} />}
              <h4 className="font-bold text-gray-900 text-lg">
                {isCritical ? '⚠️ Критический звук!' : isImportant ? '🔔 Важный звук!' : '🔊 Звук обнаружен'}
              </h4>
            </div>
            
            <p className="text-gray-800 font-semibold text-lg">{soundType}</p>
            <div className="flex items-center gap-4 mt-2">
              <p className="text-sm text-gray-700 font-medium">
                Уверенность: <span className="text-lg font-bold">{(confidence * 100).toFixed(1)}%</span>
              </p>
              <div className="flex items-center gap-1">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                <p className="text-xs text-gray-600">{deviceName}</p>
              </div>
            </div>
...
function NotificationPanel({ notifications, onClose, onMarkAsRead, onClearAll }: NotificationPanelProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col animate-in fade-in zoom-in duration-300">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {notifications.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm mt-2">Здесь появятся уведомления о важных звуках</p>
            </div>
          ) : (
...
function NotificationItem({ notification, onMarkAsRead }: NotificationItemProps) {
  ...
  return (
    <div className={`border-2 rounded-xl p-4 transition-all duration-200 hover:shadow-md ${bgColor} ${!isRead ? 'shadow-md' : 'opacity-75'}`}>
      <div className="flex items-start gap-3">
        <div className="text-2xl">{soundIcon}</div>
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-gray-900">{soundType}</h4>
            {!isRead && (
