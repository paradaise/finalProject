...
class AudioEnhancer:
    ...
    def apply_filters(self, audio_data: np.ndarray) -> np.ndarray:
        ...
        try:
            print(f"  [DEBUG] Applying filters to audio with shape: {audio_data.shape}")
            # Bandpass filter: 80Hz - 8000Hz (human speech range)
            nyquist = self.sample_rate / 2
            low = 80 / nyquist
            high = 8000 / nyquist

            print(f"  [DEBUG] Filter range: {low:.3f} - {high:.3f}")
            ...
            print(
                f"  [DEBUG] Filter applied. Original RMS: {np.sqrt(np.mean(audio_data**2)):.4f}, Filtered RMS: {np.sqrt(np.mean(filtered**2)):.4f}"
            )
    ...
    def enhance_audio(
        ...
        try:
            ...
            # 2. Simple noise reduction using moving average
            try:
                window_size = 5
            ...
            # 3. Normalization to target level
            try:
    ...
    def get_enhancement_summary(
        self, original_metrics: Dict, enhanced_metrics: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "original_rms" in original_metrics
                and "enhanced_rms" in enhanced_metrics
            ):
                rms_improvement = (
                    enhanced_metrics["enhanced_rms"] / original_metrics["original_rms"]
                    - 1
                ) * 100
            ...
            if "snr_db" in enhanced_metrics:
                ...
                improvements["signal_quality"] = self._classify_signal_quality(
                    enhanced_metrics["snr_db"]
                )

            if "clipping_ratio" in enhanced_metrics:
                improvements["clipping_status"] = (
                    "Good" if enhanced_metrics["clipping_ratio"] < 0.001 else "Detected"
                )

            improvements["overall_improvement"] = self._calculate_overall_improvement(
...
def test_audio_enhancement():
    ...
    original_metrics = {
        ...
        "clipping_ratio": float(
            np.sum(np.abs(noisy_signal) > 0.99) / len(noisy_signal)
        ),
