...
export function NotificationSettings({ onBack }: Props) {
  ...
  const [filter, setFilter] = useState<
    "all" | "notification" | "excluded" | "none"
  >("all");
  ...
  useEffect(() => {
    // Get ID of first device (for MVP)
    const loadDeviceId = async () => {
      try {
        const response: any = await apiClient.getDevices();
  ...
  const loadNotificationSettings = async (deviceId: string) => {
    try {
      ...
      settings.custom_sounds.forEach(
        (customSound: { name: string; type: string }) => {
          if (!processedSounds.has(customSound.name)) {
            // 'specific' = важные звуки (уведомления), 'excluded' = исключенные
            const notificationType =
              customSound.type === "specific" ? "notification" : "excluded";
      ...
      setNotificationSounds((prev) =>
        prev.map((s) => {
          if (s.name === soundName && !s.isCustom) {
            const types: ("notification" | "excluded")[] = [
              "notification",
              "excluded",
            ];
            const currentIndex = types.indexOf(
              s.type as "notification" | "excluded",
            );
  ...
  const deleteSound = async (soundName: string) => {
    ...
    if (
      !window.confirm(`Удалить звук "${soundName}" из настроек уведомлений?`)
    ) {
      return;
    }
  ...
  const filteredYamnetSounds = allSounds.filter(
    (sound) =>
      !notificationSounds.some(
        (ns) => ns.name.toLowerCase() === sound.toLowerCase(),
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Управление звуками для уведомлений
                </p>
              </div>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-lg font-semibold text-green-600">
                {
                  notificationSounds.filter((s) => s.type === "notification")
                    .length
                }
              </p>
            </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-gray-600 truncate">
                            {getNotificationLabel(sound.type)}
                          </p>
                        </div>
                      </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mt-4">
          ...
          <ul className="text-sm text-blue-800 space-y-1">
            <li>
              • <strong>Уведомления:</strong> Вы будете получать уведомления на
              эти звуки
            </li>
            <li>
              • <strong>Исключены:</strong> Эти звуки не будут вызывать
              уведомления
            </li>
            <li>
              • <strong>Без уведомлений:</strong> Нейтральные звуки без
              уведомлений
            </li>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-300">
            <div className="p-4 sm:p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">
                Добавить пользовательский звук
              </h3>
              <input
                type="text"
